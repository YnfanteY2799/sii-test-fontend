...
export default function MonobankCard(props: ICardProps): ReactNode {
	...
	return (
		<div className="relative w-[580px] h-[310px] cursor-pointer perspective-[1000px]" onMouseEnter={onMouse} onMouseLeave={onMouse}>
			<div
				className={`relative w-full h-full transition-transform duration-700 ease-in-out transform-3d ${isFlipped ? "rotate-y-180" : ""}`}
				...
				<div className="absolute inset-0 w-full h-full bg-gray-800 rounded-2xl border-4 border-white shadow-2xl overflow-hidden backface-hidden transform-[rotateY(180deg)]">
					...
					{/* Magnetic stripe */}
					<div className="absolute top-12 left-0 right-0 h-12 bg-black shadow-inner" />

					{/* Signature strip */}
					<div className="absolute top-28 left-8 right-8 h-12 bg-white rounded-md flex items-center justify-end px-6 shadow-md">
					...
					{/* CVV label */}
					<div className="absolute top-66 right-8 text-gray-400 text-sm">CVV</div>
